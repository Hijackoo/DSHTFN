...
class Wrapper:
    ...
    def __init__(self, model, optimizer, criterion,
        ...
        self.val_accuracies = []  # ��ʼ�� val_accuracies �б�
    ...
    def train_on_epoch(self, X, y, x_val, y_val, final_predict_result_path="Final_Predict_Result.csv", all_accuracies_path="All_Accuracies.csv"):
        # ��ÿ��ѵ���ִν���ʱִ�еĲ���
        X = X.astype("float32")
        if self.label_type == "c":
            y = y.astype("int64")
        ...
        for epoch in range(self.epochs):
            ...
            # ��ÿ�� epoch ����ʱ���㲢���� accuracy_score
            y_val_pred = self.predict(x_val).argmax(axis=1)
            accuracy = accuracy_score(y_val, y_val_pred)
            print(f"Epoch {epoch}, Validation Accuracy: {accuracy}")
            self.val_accuracies.append(accuracy)

            self.__run_callbacks__("on_epoch_end")
        np.savetxt(all_accuracies_path, np.array(self.val_accuracies), fmt='%.4f', delimiter=',')
        # �����һ�� epoch ����ʱִ�еĲ��������籣�����յ� predict ���
        y_final_pred = self.predict(x_val).argmax(axis=1)
        np.savetxt(final_predict_result_path, y_final_pred, fmt='%d', delimiter=',')
...
class Wrapper2:
    ...
    """
    def __init__(self, model, optimizer, criterion,
                 batch_size=512, epochs=1, callbacks=None, label_type="c", log_dir='runs/experiment_1',
                 device="cpu", reset_param=True, ur=0, ur_tau=0.5, **kwargs):
        self.model = model
        self.optimizer = optimizer
        self.criterion = criterion
        ...
        # self.cumulative_loss = 0.0  # 用于存储每个epoch的累计损失
        # self.num_batches = 0  # 记录每个epoch中的batch数量
    ...
    def train_on_batch(self, input, target):
        ...
        # self.num_batches += 1
        return loss.item(), outputs.argmax(dim=1).cpu().numpy()
    ...
    def fit_loader(self, train_loader):
        ...
        for e in range(self.epochs):
            self.cur_epoch = e
            self.__run_callbacks__("on_epoch_begin")
            # self.cumulative_loss = 0.0  # 每个epoch开始时重置累计损失
            # self.num_batches = 0  # 每个epoch开始时重置batch计数器
            cumulative_loss = 0.0
            num_batches = 0
            y_true = []
            y_pred = []
            ...
            for inputs in train_loader:
                ...
                self.model.train()
                # self.train_on_batch(inputs[0], inputs[1])
                loss, preds = self.train_on_batch(inputs[0], inputs[1])
                cumulative_loss += loss
                num_batches += 1
                y_true.extend(inputs[1].cpu().numpy())
                y_pred.extend(preds)
                self.__run_callbacks__("on_batch_end")
                self.cur_batch += 1

            train_avg_loss = cumulative_loss / max(num_batches, 1)
            train_acc = accuracy_score(y_true, y_pred)

            # # 在每个epoch结束后记录训练损失
            # avg_loss = self.cumulative_loss / max(self.num_batches, 1)
            # self.writer.add_scalar('Training Loss', avg_loss, self.cur_epoch)

            # 记录到SummaryWriter
            self.writer.add_scalar('Training Loss', train_avg_loss, self.cur_epoch)
            self.writer.add_scalar('Training Accuracy', train_acc, self.cur_epoch)

            # 打印出来
            print(f"Epoch {self.cur_epoch}, Training Accuracy: {train_acc}, Training Loss: {train_avg_loss}")
